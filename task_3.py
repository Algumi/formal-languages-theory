class Automatic:
    def __init__(self, **kwargs):
        ...
        elif "regexp" in kwargs.keys():
            q, e, b, s, f = self.generate_from_regexp(kwargs["regexp"])
    ...
    def generate_from_regexp(self, regexp):
        pass
...
def search_lexemes_in_text(text_filename, autos):
...
def read_grammars(filename):
    input_f = open(filename, encoding="utf-8").read()
    autos = []
    for line in input_f.split("\n"):
        auto_id, m, regexp = line.split(":")
        autos.append(Automatic(lexeme=auto_id, priority=m, regexp=regexp))
    return autos


def test_search_float_numbers_in_text(grammars=False, text_file_names=None, auto_file_names=None):
    path = "test_data/task_3/"
    ...
    if grammars:
        autos = read_grammars(path + auto_file_names)
    else:
        autos = create_autos_from_file(auto_files)
    for name in text_files:
        search_lexemes_in_text(name, autos)


test_search_float_numbers_in_text(True, ["text_with_float_numbers_1.txt"], "grammars.txt")
